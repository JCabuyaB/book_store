...
class Compra_detalle
{
    ...
    public function insert_all_bill()
    {
        ...
        try {
            // id usuario se asigna en el controlador para crear el pedido o compra
            $query_compra = "INSERT INTO tbl_compras VALUES(null, {$this->__get('id_usuario')}, CURDATE(), 'Por confirmar');";

            $insertar_compra = $connection->query($query_compra);
            ...
            $id_pedido = $connection->insert_id;
            ...
            foreach ($_SESSION['cart'] as $index => $valor) {
                ...
                $query_detalle = "INSERT INTO tbl_detalle_compras VALUES(null, {$id_pedido}, {$this->__get('id_libro')}, {$this->__get('cantidad')}, {$this->__get('total')});";
                $insertar_detalle = $connection->query($query_detalle);
